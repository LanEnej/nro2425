...
int main() {
    ...
    for (int ii = 0; ii < 2000; ii++)
    {
        ...
        for (int iter = 0; iter < 2000; iter++) { // Iteracija metode
            for (int i = 0; i < n; i++) {         // Iteracija čez vse elemente rešitve T
                double sigma = 0.0;

                // Paralelizacija notranje zanke z uporabo OpenMP
#pragma omp parallel for reduction(+:sigma)
                for (int j = 0; j < n; j++) {
                    if (j != i) { // Preskočimo diagonalni element
                        sigma += A[i][j] * T[j];
                    }
                }

                // Posodobitev T[i] (sekvenčna, da ohranimo pravilnost Gauss-Seidelove metode)
                T[i] = (b[i] - sigma) / A[i][i];
            }
        }
