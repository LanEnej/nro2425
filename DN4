...
int main() {
    ...
    {
        // Gauss-Seidelova metoda z OpenMP
        for (int i = 0; i < n; i++) { // Iteracija po vrsticah matrike A
            double sum = 0.0;

            // Paralelizirana notranja zanka z OpenMP
            #pragma omp parallel for default(shared) private(sum) reduction(+:sum)
            for (int k = 0; k < n; k++) { // Iteracija po stolpcih matrike A
                if (k != i) { // Izključimo diagonalo
                    sum += A[i][k] * T[k];
                }
            }

            // Sekvenčna posodobitev trenutnega elementa rešitve
            double new_value = (b[i] - sum) / A[i][i];
            T[i] = new_value; // Shrani novo vrednost vektorja T
        }

    }
    /*Metoda je delno paralelizirana.Paralelizacija je mogoča v notranji zanki, kjer se računa vsota sigma.
        To je zato, ker je vsak člen v vsoti (A[i][j] * T[j]) popolnoma neodvisen od ostalih in zato omogoča
        varno uporabo OpenMP za paralelno izvajanje.

        Zunanja zanka ni paralelizirana, saj Gauss-Seidelova metoda temelji na sekvenčni posodobitvi vrednosti
        T[i]. Posodobitev T[i] v trenutni iteraciji uporablja že posodobljene vrednosti T[j], kjer je j < i.
        To pomeni, da je zunanja zanka odvisna od podatkov iz prejšnjih iteracij, kar preprečuje popolno
        paralelizacijo metode.*/
